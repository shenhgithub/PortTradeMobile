...
class PacketReader {
    ...
    private static final String LOG_TAG = LogUtil.makeLogTag(PacketReader.class);
...
/*    public void startKeepHeartBeat(XmppManager xmppManager){
    ...
    void notifyConnectionError(Exception e) {
        Log.d(LOG_TAG, "notifyConnectionError", e);
        done = true;
        ...
        try {
            ...
            do {
                if (eventType == XmlPullParser.START_TAG) {
                    Log.d(LOG_TAG, "starttag...parser..." + parser.getName());
                    ...
                    Log.d(LOG_TAG, "end...tag...parser..." + parser.getName());
    ...
    private void parseFeatures(XmlPullParser parser) throws Exception {
        ...
        while (!done) {
            ...
            if (eventType == XmlPullParser.START_TAG) {
                if (parser.getName().equals("starttls")) {
                    ...
                    // the server
                    connection.getSASLAuthentication().setAvailableSASLMethods(PacketParserUtils.parseMechanisms(parser));
        ...
        if (!connection.isSecureConnection()) {
            if (!startTLSReceived && connection.getConfiguration().getSecurityMode() == ConnectionConfiguration.SecurityMode.required) {
        ...
        if (!startTLSReceived || connection.getConfiguration().getSecurityMode() == ConnectionConfiguration.SecurityMode.disabled) {
            releaseConnectionIDLock();
        }
    }
