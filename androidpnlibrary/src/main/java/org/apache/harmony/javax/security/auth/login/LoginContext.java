...
public class LoginContext {
    ...
    private final class Module {
         ...
         * initialize().
         */
        void create(Subject subject, CallbackHandler callbackHandler, Map<String, ?> sharedState) throws LoginException {
            String klassName = entry.getLoginModuleName();
            if (klass == null) {
                try {
                    klass = Class.forName(klassName, false, contextClassLoader);
                } catch (ClassNotFoundException ex) {
                    throw (LoginException) new LoginException("auth.39 " + klassName).initCause(ex); //$NON-NLS-1$
                }
            }
            ...
            if (module == null) {
                try {
                    ...
                    throw (LoginException) new LoginException("auth.3A " + klassName) //$NON-NLS-1$
                            .initCause(ex);
