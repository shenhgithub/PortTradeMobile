...
public final class Subject implements Serializable {
    ...
    private static final AuthPermission _AS_PRIVILEGED = new AuthPermission("doAsPrivileged"); //$NON-NLS-1$

    private static final AuthPermission _SUBJECT = new AuthPermission("getSubject"); //$NON-NLS-1$

    private static final AuthPermission _PRINCIPALS = new AuthPermission("modifyPrincipals"); //$NON-NLS-1$

    private static final AuthPermission _PRIVATE_CREDENTIALS = new AuthPermission("modifyPrivateCredentials"); //$NON-NLS-1$

    private static final AuthPermission _PUBLIC_CREDENTIALS = new AuthPermission("modifyPublicCredentials"); //$NON-NLS-1$

    private static final AuthPermission _READ_ONLY = new AuthPermission("setReadOnly"); //$NON-NLS-1$
     ...
     *                if {@code null} a new {@link java.security.AccessControlContext} is
     *                instantiated.
     *
     * @return the {@code Object} returned when running the {@code action}.
     */
     ...
     * @return the {@code Object} returned when running the {@code action}.
     *
     * @throws java.security.PrivilegedActionException if running the {@code action} throws an exception.
     */
    @SuppressWarnings("unchecked")
    ...
    /**
     * Returns this {@code Subject}'s {@link java.security.Principal}.
     *
     * @return this {@code Subject}'s {@link java.security.Principal}.
     */
    ...
    /**
     * Returns this {@code Subject}'s {@link java.security.Principal} which is a subclass of
     * the {@code Class} provided.
     *
     * @param c the {@code Class} as a criteria which the {@code Principal}
     *          returned must satisfy.
     *
     * @return this {@code Subject}'s {@link java.security.Principal}. Modifications to the
     ...
     * java.security.Principal} sets. After setting it to read-only this {@code Subject} can
            ...
            if (permission == _PRIVATE_CREDENTIALS) {
                ...
                return new SecureIterator(elements.iterator()) {
                    ...
                    public SST next() {
                        ...
                        checkPermission(new PrivateCredentialPermission(obj.getClass().getName(), principals));
                        return obj;
                    }
                };
            }
            ...
            AbstractSet<E> s = new AbstractSet<E>() {
                ...
                public boolean add(E o) {
                    ...
                    if (!c.isAssignableFrom(o.getClass())) {
                        throw new IllegalArgumentException("auth.0C " + c.getName()); //$NON-NLS-1$
                    }
