 ...
 * @since 1.0
 */
public class CommonFunctionFragment extends BaseIncludeWebFragment implements TitleHandle {
        ...
        getWebView().setWebChromeClient(new MobileWebChromeClient(getActivity()) {
            ...
            public void onReceivedTitle(WebView view, String title) {
                ...
                if (title != null && title.length() > 0) {
                    ...
                    if (title.equals(getString(org.mobile.mobilelibrary.R.string.web_view_load_error_title))) {
                        // 当前页面为错误页面，替换title为本功能页名称
                        title = getString(R.string.action_common_function);
                    }
    ...
    protected WebView onInitWebView(View rootView) {
        // 使用带有下拉刷新的WebView
        pullRefreshWebView = (PullToRefreshWebView) rootView.findViewById(R.id.with_pull_refresh_webView);
    ...
    protected String onCreateUrl() {
        ...
        if (MemoryConfig.getConfig().isLogin()) {
            // 已登录传入用户标识
            urlParam += MemoryConfig.getConfig().getUserID();
        } else {
            // 未登录传入0
            urlParam += "0";
        }
        ...
        // 加载功能页
        return StaticValue.COMMON_FUNCTION_URL + urlParam;
    }
